...
function phantom_terminal(){
    echo -e "\n${turquoiseColour}$(printf '=%.0s' {1..40}) [::] PHANTOM TERMINAL [::] $(printf '=%.0s' {1..51})${endColour}\n"
...
EOF
    ...
    echo -e "\n${turquoiseColour}$(printf '=%.0s' {1..40}) [::] PHANTOM TERMINAL DEPLOYED [::] $(printf '=%.0s' {1..42})${endColour}\n"
